...
function insurance_ancora_theme_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = INSURANCE_ANCORA_STORAGE['co_add_params']!=undefined ? INSURANCE_ANCORA_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#insurance_ancora-customizer-styles-'+option).length > 0 ? jQuery('#insurance_ancora-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="insurance_ancora-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
