...
if ( !function_exists( 'insurance_ancora_template_testimonials_1_theme_setup' ) ) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_template_testimonials_1_theme_setup', 1 );
	function insurance_ancora_template_testimonials_1_theme_setup() {
		insurance_ancora_add_template(array(
			'layout' => 'testimonials-1',
			'template' => 'testimonials-1',
			'mode'   => 'testimonials',
			'title'  => esc_html__('Testimonials /Style 1/', 'insurance-ancora'),
...
// Template output
if ( !function_exists( 'insurance_ancora_template_testimonials_1_output' ) ) {
	function insurance_ancora_template_testimonials_1_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?> class="sc_testimonial_item<?php echo (!empty($post_options['tag_class']) ? ' '.esc_attr($post_options['tag_class']) : ''); ?>"<?php echo !empty($post_options['tag_css']) ? ' style="'.esc_attr($post_options['tag_css']).'"' : '';?>>
				<div class="sc_testimonial_content"><?php insurance_ancora_show_layout($post_data['post_content']); ?></div>
				<?php if ($post_options['photo']) { ?>
				<div class="sc_testimonial_avatar"><?php insurance_ancora_show_layout($post_options['photo']); ?></div>
				<?php } ?>
