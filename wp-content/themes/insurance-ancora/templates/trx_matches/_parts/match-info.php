...
$match_date = 	!empty($post_meta['match_date']) && !insurance_ancora_param_is_inherit($post_meta['match_date']) ? date("d M", strtotime($post_meta['match_date'])) : '';	
$match_time = 	!empty($post_meta['match_time']) && !insurance_ancora_param_is_inherit($post_meta['match_time']) ? $post_meta['match_time'] : '';	
$match_score = 	!empty($post_meta['match_score']) && !insurance_ancora_param_is_inherit($post_meta['match_score']) ? $post_meta['match_score'] : '0-0';
$match_passed = false;
...
if (!empty($first_player) && $first_player!='none' && ($first_player_obj = (intval($first_player) > 0 ? get_post($first_player, OBJECT) : get_page_by_title($first_player, OBJECT, 'players'))) != null) {
	$first_player_post_meta = get_post_meta($first_player_obj->ID, insurance_ancora_storage_get('options_prefix') . '_post_options', true);
	...
	$first_player_photo = insurance_ancora_get_resized_image_tag($first_player_photo, $thumb_sizes['w'], $thumb_sizes['h']);
}
...
if (!empty($second_player) && $second_player!='none' && ($second_player_obj = (intval($second_player) > 0 ? get_post($second_player, OBJECT) : get_page_by_title($second_player, OBJECT, 'players'))) != null) {
	$second_player_post_meta = get_post_meta($second_player_obj->ID, insurance_ancora_storage_get('options_prefix') . '_post_options', true);
	...
	$second_player_photo = insurance_ancora_get_resized_image_tag($second_player_photo, $thumb_sizes['w'], $thumb_sizes['h']);
}
...
if (!empty($first_player) && $first_player!='none' && ($first_player_obj = (intval($first_player) > 0 ? get_post($first_player, OBJECT) : get_page_by_title($first_player, OBJECT, 'players'))) != null
	...
	$first_player_points = get_post_meta($first_player_obj->ID, insurance_ancora_storage_get('options_prefix').'_points', true);
	$second_player_points = get_post_meta($second_player_obj->ID, insurance_ancora_storage_get('options_prefix').'_points', true);
?>
	<div class="match_block"><?php
		// First player
		?><div class="player player_first">
			<div class="player_country"><?php insurance_ancora_show_layout($first_player_country); ?></div>
			<div class="player_photo">
				<a class="hover_icon hover_icon_view" href="<?php echo esc_url($first_player_link); ?>" title="<?php echo esc_attr($first_player_name); ?>">
					<?php insurance_ancora_show_layout($first_player_photo); ?>
				</a>				
			</div>
			<div class="player_name"><a href="<?php insurance_ancora_show_layout($first_player_link); ?>"><?php insurance_ancora_show_layout($first_player_name);  ?></a></div>
		</div><?php
		// Score
		?><div class="match_info">
			<div class="match_date"><?php insurance_ancora_show_layout($match_date); ?> <?php insurance_ancora_show_layout($match_time); ?></div>
		...
		?><div class="player player_second">
			<div class="player_country"><?php insurance_ancora_show_layout($second_player_country); ?></div>
			<div class="player_photo">
				<a class="hover_icon hover_icon_view" href="<?php echo esc_url($second_player_link); ?>" title="<?php echo esc_attr($second_player_name); ?>">
					<?php insurance_ancora_show_layout($second_player_photo); ?>
				</a>				
			</div>
			<div class="player_name"><a href="<?php insurance_ancora_show_layout($second_player_link); ?>"><?php insurance_ancora_show_layout($second_player_name);  ?></a></div>
		</div>
	</div>
<?php
}
