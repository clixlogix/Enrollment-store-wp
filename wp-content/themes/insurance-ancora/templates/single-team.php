...
if ( !function_exists( 'insurance_ancora_template_single_team_theme_setup' ) ) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_template_single_team_theme_setup', 1 );
	function insurance_ancora_template_single_team_theme_setup() {
		insurance_ancora_add_template(array(
			'layout' => 'single-team',
			'mode'   => 'team',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single Team member', 'insurance-ancora'),
...
// Template output
if ( !function_exists( 'insurance_ancora_template_single_team_output' ) ) {
	function insurance_ancora_template_single_team_output($post_options, $post_data) {
		...
		if ($show_title && $post_options['location'] != 'center' && insurance_ancora_get_custom_option('show_page_title')=='no') {
			?>
			<<?php echo esc_html($title_tag); ?> itemprop="name" class="post_title entry-title"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php insurance_ancora_show_layout($post_data['post_title']); ?></<?php echo esc_html($title_tag); ?>>
			<?php 
		}
