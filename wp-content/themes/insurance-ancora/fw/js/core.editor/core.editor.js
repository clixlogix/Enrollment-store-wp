jQuery(document).ready(function () {
	...
	jQuery('#frontend_editor_icon_edit').on('click', function (e) {
		...
		jQuery('#frontend_editor_button_cancel').val(INSURANCE_ANCORA_STORAGE['strings']['editor_caption_cancel']);
	...
	jQuery('#frontend_editor_button_cancel').on('click', function (e) {
		"use strict";
		if (jQuery(this).val() == INSURANCE_ANCORA_STORAGE['strings']['editor_caption_close'])
	...
	jQuery('#frontend_editor_button_save').on('click', function (e) {
		...
		jQuery.post(INSURANCE_ANCORA_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error == '') {
				insurance_ancora_message_success('', INSURANCE_ANCORA_STORAGE['strings']['editor_save_success']);
				jQuery('#frontend_editor_button_cancel').val(INSURANCE_ANCORA_STORAGE['strings']['editor_caption_close']);
			} else {
				insurance_ancora_message_warning(rez.error, INSURANCE_ANCORA_STORAGE['strings']['editor_save_error']);
			}
		});
		e.preventDefault();
		return false;
	...
	jQuery('#frontend_editor_icon_delete').on('click', function (e) {
		"use strict";
		insurance_ancora_message_confirm(INSURANCE_ANCORA_STORAGE['strings']['editor_delete_post'], INSURANCE_ANCORA_STORAGE['strings']['editor_delete_post_header'], function(btn) {
			"use strict";
			if (btn != 1) return;
			var data = {
			...
			jQuery.post(INSURANCE_ANCORA_STORAGE['ajax_url'], data, function(response) {
				...
				if (rez.error == '') {
					insurance_ancora_message_success('', INSURANCE_ANCORA_STORAGE['strings']['editor_delete_success']);
					setTimeout(function() { 
						window.location.href = INSURANCE_ANCORA_STORAGE['site_url'];
						}, 1000);
				} else {
					insurance_ancora_message_warning(rez.error, INSURANCE_ANCORA_STORAGE['strings']['editor_delete_error']);
				}
			});
