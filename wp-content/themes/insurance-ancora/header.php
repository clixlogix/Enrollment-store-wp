...
<body <?php body_class();?>>
	...
	<?php get_template_part(insurance_ancora_get_file_slug('sidebar_outer.php')); ?>

	<?php
		$body_style  = insurance_ancora_get_custom_option('body_style');
		$class = $style = '';
		if (insurance_ancora_get_custom_option('bg_custom')=='yes' && ($body_style=='boxed' || insurance_ancora_get_custom_option('bg_image_load')=='always')) {
			if (($img = insurance_ancora_get_custom_option('bg_image_custom')) != '')
				$style = 'background: url('.esc_url($img).') ' . str_replace('_', ' ', insurance_ancora_get_custom_option('bg_image_custom_position')) . ' no-repeat fixed;';
			else if (($img = insurance_ancora_get_custom_option('bg_pattern_custom')) != '')
				$style = 'background: url('.esc_url($img).') 0 0 repeat fixed;';
			else if (($img = insurance_ancora_get_custom_option('bg_image')) > 0)
				$class = 'bg_image_'.($img);
			else if (($img = insurance_ancora_get_custom_option('bg_pattern')) > 0)
				$class = 'bg_pattern_'.($img);
			if (($img = insurance_ancora_get_custom_option('bg_color')) != '')
	...
	<div class="body_wrap<?php echo !empty($class) ? ' '.esc_attr($class) : ''; ?>"<?php echo !empty($style) ? ' style="'.esc_attr($style).'"' : ''; ?>>
		...
		<?php
		$video_bg_show = insurance_ancora_get_custom_option('show_video_bg')=='yes';
		$youtube = insurance_ancora_get_custom_option('video_bg_youtube_code');
		$video   = insurance_ancora_get_custom_option('video_bg_url');
		$overlay = insurance_ancora_get_custom_option('video_bg_overlay')=='yes';
		if ($video_bg_show && (!empty($youtube) || !empty($video))) {
			if (!empty($youtube)) {
				...
				$ext = insurance_ancora_get_file_ext($video);
		...
		<div class="page_wrap">
			...
			<?php
			$top_panel_style = insurance_ancora_get_custom_option('top_panel_style');
			$top_panel_position = insurance_ancora_get_custom_option('top_panel_position');
			$top_panel_scheme = insurance_ancora_get_custom_option('top_panel_scheme');
			// Top panel 'Above' or 'Over'
			if (in_array($top_panel_position, array('above', 'over'))) {
				insurance_ancora_show_post_layout(array(
					'layout' => $top_panel_style,
					'position' => $top_panel_position,
					'scheme' => $top_panel_scheme
					), false);
				// Mobile Menu
				get_template_part(insurance_ancora_get_file_slug('templates/headers/_parts/header-mobile.php'));
			}

			// Slider
			get_template_part(insurance_ancora_get_file_slug('templates/headers/_parts/slider.php'));
			...
			$show_title = insurance_ancora_get_custom_option('show_page_title')=='yes';
			$show_navi = apply_filters('insurance_ancora_filter_show_post_navi', false);
			$show_breadcrumbs = insurance_ancora_get_custom_option('show_breadcrumbs')=='yes';
						...
						<div class="content_wrap">
							...
							if ($show_title) {
								if ($show_navi) {
									...
									?><h4 class="page_title"><?php echo wp_kses_post(insurance_ancora_get_blog_title()); ?></h4><?php
								}
							}
							if ($show_breadcrumbs) {
								?><div class="breadcrumbs"><?php if (!is_404()) insurance_ancora_show_breadcrumbs(); ?></div><?php
							}
							?>
						</div>
					</div>
			...
			<div class="page_content_wrap page_paddings_<?php echo esc_attr(insurance_ancora_get_custom_option('body_paddings')); ?>">

				<?php
				// Content and sidebar wrapper
				if ($body_style!='fullscreen') insurance_ancora_open_wrapper('<div class="content_wrap">');
				
				// Main content wrapper
				insurance_ancora_open_wrapper('<div class="content">');
				?>