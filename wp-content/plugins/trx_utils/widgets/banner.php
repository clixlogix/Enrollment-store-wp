...
// Theme init
if (!function_exists('insurance_ancora_widget_banner_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_widget_banner_theme_setup', 1 );
	function insurance_ancora_widget_banner_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_widget_banner_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('insurance_ancora_widget_banner_load')) {
	add_action( 'widgets_init', 'insurance_ancora_widget_banner_load' );
	function insurance_ancora_widget_banner_load() {
		register_widget( 'insurance_ancora_widget_banner' );
	}
}

// Widget Class
class insurance_ancora_widget_banner extends WP_Widget {

	function __construct() {
		$widget_ops = array( 'classname' => 'widget_banner', 'description' => esc_html__('Banner', 'trx_utils') );
		parent::__construct( 'insurance_ancora_widget_banner', esc_html__('Insurance - Banner', 'trx_utils'), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<div class="widget_banner_inner">
			...
			if ($banner_image!='') {
				...
				$attr = insurance_ancora_getimagesize($banner_image);
			...
			if ($banner_code!='') {
				echo force_balance_tags(insurance_ancora_substitute_all($banner_code));
			}
			?>
		</div>
		<?php
	...
	function form( $instance ) {
		...
		<p>
            ...
            <?php
			insurance_ancora_show_layout(insurance_ancora_show_custom_field($this->get_field_id( 'banner_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'banner_image' )), null));
			if ($banner_image) {
			?>
...
*/
if ( !function_exists( 'insurance_ancora_sc_widget_banner' ) ) {
	function insurance_ancora_sc_widget_banner($atts, $content=null){	
		...
		extract($atts);
		$type = 'insurance_ancora_widget_banner';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			...
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_banner' 
								. (insurance_ancora_exists_visual_composer() ? ' vc_widget_banner wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, insurance_ancora_prepare_widgets_args(insurance_ancora_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_banner', 'widget_banner') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_widget_banner', $atts, $content);
	}
	add_shortcode("trx_widget_banner", "insurance_ancora_sc_widget_banner");
}


// Add [trx_widget_banner] in the VC shortcodes list
if (!function_exists('insurance_ancora_widget_banner_reg_shortcodes_vc')) {
	function insurance_ancora_widget_banner_reg_shortcodes_vc() {
