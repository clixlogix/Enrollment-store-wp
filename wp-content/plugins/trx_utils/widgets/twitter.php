...
// Theme init
if (!function_exists('insurance_ancora_widget_twitter_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_widget_twitter_theme_setup', 1 );
	function insurance_ancora_widget_twitter_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_widget_twitter_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('insurance_ancora_widget_twitter_load')) {
	add_action( 'widgets_init', 'insurance_ancora_widget_twitter_load' );
	function insurance_ancora_widget_twitter_load() {
		register_widget( 'insurance_ancora_widget_twitter' );
	}
}

// Widget Class
class insurance_ancora_widget_twitter extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'insurance_ancora_widget_twitter', esc_html__('Insurance - Twitter', 'trx_utils'), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$twitter_username = !empty($instance['twitter_username']) ? $instance['twitter_username'] : insurance_ancora_get_theme_option('twitter_username');
		$twitter_consumer_key = !empty($instance['twitter_consumer_key']) ? $instance['twitter_consumer_key'] : insurance_ancora_get_theme_option('twitter_consumer_key');
		$twitter_consumer_secret = !empty($instance['twitter_consumer_secret']) ? $instance['twitter_consumer_secret'] : insurance_ancora_get_theme_option('twitter_consumer_secret');
		$twitter_token_key = !empty($instance['twitter_token_key']) ? $instance['twitter_token_key'] : insurance_ancora_get_theme_option('twitter_token_key');
		$twitter_token_secret = !empty($instance['twitter_token_secret']) ? $instance['twitter_token_secret'] : insurance_ancora_get_theme_option('twitter_token_secret');
		$twitter_count = max(1, !empty($instance['twitter_count']) ? (int) $instance['twitter_count'] : (int) insurance_ancora_get_theme_option('twitter_count'));	

		if (empty($twitter_consumer_key) || empty($twitter_consumer_secret) || empty($twitter_token_key) || empty($twitter_token_secret)) return;
		
		$data = insurance_ancora_get_twitter_data(array(
			'mode'            => 'user_timeline',
		...
		if (is_array($data) && count($data) > 0) {
			foreach ($data as $tweet) {
				if (insurance_ancora_substr($tweet['text'], 0, 1)=='@') continue;
				$output .= '<li class="theme_text' . ($cnt==$twitter_count-1 ? ' last' : '') . '"><a href="' . esc_url('https://twitter.com/'.($twitter_username)) . '" class="username" target="_blank">@' . ($tweet['user']['screen_name']) . '</a> ' . force_balance_tags(insurance_ancora_prepare_twitter_text($tweet)) . '</li>';
...
*/
if ( !function_exists( 'insurance_ancora_sc_widget_twitter' ) ) {
	function insurance_ancora_sc_widget_twitter($atts, $content=null){	
		...
		extract($atts);
		$type = 'insurance_ancora_widget_twitter';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			...
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_twitter' 
								. (insurance_ancora_exists_visual_composer() ? ' vc_widget_twitter wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, insurance_ancora_prepare_widgets_args(insurance_ancora_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_twitter', 'widget_twitter') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_widget_twitter', $atts, $content);
	}
	add_shortcode("trx_widget_twitter", "insurance_ancora_sc_widget_twitter");
}


// Add [trx_widget_twitter] in the VC shortcodes list
if (!function_exists('insurance_ancora_widget_twitter_reg_shortcodes_vc')) {
	function insurance_ancora_widget_twitter_reg_shortcodes_vc() {
