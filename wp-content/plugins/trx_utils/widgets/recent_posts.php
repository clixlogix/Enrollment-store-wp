...
// Theme init
if (!function_exists('insurance_ancora_widget_recent_posts_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_widget_recent_posts_theme_setup', 1 );
	function insurance_ancora_widget_recent_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_widget_recent_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('insurance_ancora_widget_recent_posts_load')) {
	add_action( 'widgets_init', 'insurance_ancora_widget_recent_posts_load' );
	function insurance_ancora_widget_recent_posts_load() {
		register_widget('insurance_ancora_widget_recent_posts');
	}
}

// Widget Class
class insurance_ancora_widget_recent_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_posts', 'description' => esc_html__('The recent blog posts (extended)', 'trx_utils'));
		parent::__construct( 'insurance_ancora_widget_recent_posts', esc_html__('Insurance - Recent Posts', 'trx_utils'), $widget_ops );

		// Add thumb sizes into list
		insurance_ancora_add_thumb_sizes( array( 'layout' => 'widgets', 'w' => 75, 'h' => 75, 'title'=>esc_html__('Widgets', 'trx_utils') ) );
	}
	...
	function form($instance) {
		...
		$counters = array_merge(array('hide' => esc_html__('Hide', 'trx_utils')), insurance_ancora_get_list_blog_counters(false));
		?>
...
*/
if ( !function_exists( 'insurance_ancora_sc_widget_recent_posts' ) ) {
	function insurance_ancora_sc_widget_recent_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'insurance_ancora_widget_recent_posts';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_recent_posts' 
								. (insurance_ancora_exists_visual_composer() ? ' vc_widget_recent_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, insurance_ancora_prepare_widgets_args(insurance_ancora_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_posts', 'widget_recent_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_widget_recent_posts', $atts, $content);
	}
	add_shortcode("trx_widget_recent_posts", "insurance_ancora_sc_widget_recent_posts");
}


// Add [trx_widget_recent_posts] in the VC shortcodes list
if (!function_exists('insurance_ancora_widget_recent_posts_reg_shortcodes_vc')) {
	function insurance_ancora_widget_recent_posts_reg_shortcodes_vc() {
