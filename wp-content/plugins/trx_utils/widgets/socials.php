...
// Theme init
if (!function_exists('insurance_ancora_widget_socials_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_widget_socials_theme_setup', 1 );
	function insurance_ancora_widget_socials_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_widget_socials_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('insurance_ancora_widget_socials_load')) {
	add_action( 'widgets_init', 'insurance_ancora_widget_socials_load' );
	function insurance_ancora_widget_socials_load() {
		register_widget( 'insurance_ancora_widget_socials' );
	}
}

// Widget Class
class insurance_ancora_widget_socials extends WP_Widget {

	function __construct() {
		$widget_ops = array( 'classname' => 'widget_socials', 'description' => esc_html__('Show site logo and social links', 'trx_utils') );
		parent::__construct( 'insurance_ancora_widget_socials', esc_html__('Insurance - Show logo and social links', 'trx_utils'), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$text = isset($instance['text']) ? insurance_ancora_do_shortcode($instance['text']) : '';
            ...
            <?php
				if ($show_logo) {
					...
					if ($logo_image || $logo_text)
						insurance_ancora_show_logo($logo_image, false, false, false, $logo_text, $logo_slogan);
				}
				...
				if ($show_icons) {
					insurance_ancora_show_layout(insurance_ancora_sc_socials(array('size' => "tiny", 'shape' => 'round')));
				}
			?>
		</div>
	...
	function form( $instance ) {
		...
		<p>
            ...
            <?php
			insurance_ancora_show_layout(insurance_ancora_show_custom_field($this->get_field_id( 'logo_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'logo_image' )), null));
			if ($logo_image) {
			?>
...
*/
if ( !function_exists( 'insurance_ancora_sc_widget_socials' ) ) {
	function insurance_ancora_sc_widget_socials($atts, $content=null){	
		...
		extract($atts);
		$type = 'insurance_ancora_widget_socials';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_socials' 
								. (insurance_ancora_exists_visual_composer() ? ' vc_widget_socials wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, insurance_ancora_prepare_widgets_args(insurance_ancora_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_socials', 'widget_socials') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_widget_socials', $atts, $content);
	}
	add_shortcode("trx_widget_socials", "insurance_ancora_sc_widget_socials");
}


// Add [trx_widget_socials] in the VC shortcodes list
if (!function_exists('insurance_ancora_widget_socials_reg_shortcodes_vc')) {
	function insurance_ancora_widget_socials_reg_shortcodes_vc() {
