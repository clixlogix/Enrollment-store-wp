...
// Theme init
if (!function_exists('insurance_ancora_widget_top10_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_widget_top10_theme_setup', 1 );
	function insurance_ancora_widget_top10_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_widget_top10_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('insurance_ancora_widget_top10_load')) {
	add_action( 'widgets_init', 'insurance_ancora_widget_top10_load' );
	function insurance_ancora_widget_top10_load() {
		register_widget('insurance_ancora_widget_top10');
	}
}

// Widget Class
class insurance_ancora_widget_top10 extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'insurance_ancora_widget_top10', esc_html__('Insurance - Top 10 Posts', 'trx_utils'), $widget_ops );
	...
	function widget($args, $instance) {
		...
		for ($i=0; $i<2; $i++) {
			...
			$post_rating = insurance_ancora_storage_get('options_prefix').'_reviews_avg'.($i==0 ? '' : '2');
		...
		if ( !empty($titles) ) {
			...
			?><div id="<?php echo esc_attr($id); ?>" class="widget_top10_content sc_tabs sc_tabs_style_2 no_jquery_ui">
				<ul class="widget_top10_tab_titles sc_tabs_titles"><?php insurance_ancora_show_layout($titles); ?></ul><?php
				insurance_ancora_show_layout($content);
			?></div><?php
...
*/
if ( !function_exists( 'insurance_ancora_sc_widget_top10' ) ) {
	function insurance_ancora_sc_widget_top10($atts, $content=null){	
		...
		extract($atts);
		$type = 'insurance_ancora_widget_top10';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_top10' 
								. (insurance_ancora_exists_visual_composer() ? ' vc_widget_top10 wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, insurance_ancora_prepare_widgets_args(insurance_ancora_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_top10', 'widget_top10') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_widget_top10', $atts, $content);
	}
	add_shortcode("trx_widget_top10", "insurance_ancora_sc_widget_top10");
}


// Add [trx_widget_top10] in the VC shortcodes list
if (!function_exists('insurance_ancora_widget_top10_reg_shortcodes_vc')) {
	function insurance_ancora_widget_top10_reg_shortcodes_vc() {
