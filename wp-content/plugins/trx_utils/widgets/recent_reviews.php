...
// Theme init
if (!function_exists('insurance_ancora_widget_recent_reviews_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_widget_recent_reviews_theme_setup', 1 );
	function insurance_ancora_widget_recent_reviews_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_widget_recent_reviews_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('insurance_ancora_widget_recent_reviews_load')) {
	add_action( 'widgets_init', 'insurance_ancora_widget_recent_reviews_load' );
	function insurance_ancora_widget_recent_reviews_load() {
		register_widget('insurance_ancora_widget_recent_reviews');
	}
}

// Widget Class
class insurance_ancora_widget_recent_reviews extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_reviews', 'description' => esc_html__('Recent reviews', 'trx_utils'));
		parent::__construct( 'insurance_ancora_widget_recent_reviews', esc_html__('Insurance - Recent reviews', 'trx_utils'), $widget_ops );
...
*/
if ( !function_exists( 'insurance_ancora_sc_widget_recent_reviews' ) ) {
	function insurance_ancora_sc_widget_recent_reviews($atts, $content=null){	
		...
		extract($atts);
		$type = 'insurance_ancora_widget_recent_reviews';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '">';
			ob_start();
			the_widget( $type, $atts, insurance_ancora_prepare_widgets_args(insurance_ancora_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_reviews', 'widget_recent_reviews') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_widget_recent_reviews', $atts, $content);
	}
	add_shortcode("trx_widget_recent_reviews", "insurance_ancora_sc_widget_recent_reviews");
}


// Add [trx_widget_recent_reviews] in the VC shortcodes list
if (!function_exists('insurance_ancora_widget_recent_reviews_reg_shortcodes_vc')) {
	function insurance_ancora_widget_recent_reviews_reg_shortcodes_vc() {
