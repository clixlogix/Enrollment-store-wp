...
// Theme init
if (!function_exists('insurance_ancora_po_composer_theme_setup')) {
	add_action( 'insurance_ancora_action_after_init_theme', 'insurance_ancora_po_composer_theme_setup' );		// Fire this action after load theme options
	function insurance_ancora_po_composer_theme_setup() {
		if (is_admin() && current_user_can('manage_options') && insurance_ancora_get_theme_option('admin_po_composer')=='yes') {
			new insurance_ancora_po_composer();
		}
	}
}


class insurance_ancora_po_composer {
	...
	function build_page() {
		...
		<div class="po_composer">
			<h2 class="po_composer_title"><?php esc_html_e('Insurance .PO-files Composer', 'trx_utils'); ?></h2>
	...
	function actions_handler() {
		if (!empty($this->po_src)) {
			if ( !wp_verify_nonce( insurance_ancora_get_value_gp('nonce'), admin_url() ) ) {
			...
			if (empty($this->error)) {
				...
				if (is_writeable($po_temp)) {
					insurance_ancora_fpc($po_temp, $this->po_text);
				...
				if (is_writeable($mo_temp)) {
					insurance_ancora_fpc($mo_temp, $this->mo_text);
				...
				if (empty($this->po_src2) && !empty($_POST['po_text']) && substr($this->po_src, 0, 7)=='parent_' || substr($this->po_src, 0, 6)=='child_') {
					...
					if (is_writeable($po_file)) {
						insurance_ancora_fpc($po_file, $this->po_text);
						...
						if (is_writeable($mo_file)) {
							insurance_ancora_fpc($mo_file, $this->mo_text);
	...
	function load_po($suffix='') {
		...
		do {
			...
			if ($_POST['po_src'.trim($suffix)] == 'upload_') {
				$rez['data'] = isset($_FILES['po_file'.($suffix)]['tmp_name']) && file_exists($_FILES['po_file'.($suffix)]['tmp_name']) ? insurance_ancora_fga($_FILES['po_file'.($suffix)]['tmp_name']) : '';
				if (empty($rez['data'])) {
			...
			// or load file from 'languages' folder
				...
				$name = insurance_ancora_esc(substr($_POST['po_src'.($suffix)], strpos($_POST['po_src'.($suffix)], '_')+1));
				$rez['data'] = file_exists(($dir).'/'.($name).'.po') ? insurance_ancora_fga(($dir).'/'.($name).'.po') : '';
				if (empty($rez['data'])) {
