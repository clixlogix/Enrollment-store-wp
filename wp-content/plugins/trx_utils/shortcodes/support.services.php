<?php
/**
 * InsuranceAncora Framework: Services support
 *
...
// Theme init
if (!function_exists('insurance_ancora_services_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_services_theme_setup',1 );
	function insurance_ancora_services_theme_setup() {
		
		// Detect current page type, taxonomy and title (for custom post_types use priority < 10 to fire it handles early, than for standard post types)
		add_filter('insurance_ancora_filter_get_blog_type',			'insurance_ancora_services_get_blog_type', 9, 2);
		add_filter('insurance_ancora_filter_get_blog_title',		'insurance_ancora_services_get_blog_title', 9, 2);
		add_filter('insurance_ancora_filter_get_current_taxonomy',	'insurance_ancora_services_get_current_taxonomy', 9, 2);
		add_filter('insurance_ancora_filter_is_taxonomy',			'insurance_ancora_services_is_taxonomy', 9, 2);
		add_filter('insurance_ancora_filter_get_stream_page_title',	'insurance_ancora_services_get_stream_page_title', 9, 2);
		add_filter('insurance_ancora_filter_get_stream_page_link',	'insurance_ancora_services_get_stream_page_link', 9, 2);
		add_filter('insurance_ancora_filter_get_stream_page_id',	'insurance_ancora_services_get_stream_page_id', 9, 2);
		add_filter('insurance_ancora_filter_query_add_filters',		'insurance_ancora_services_query_add_filters', 9, 2);
		add_filter('insurance_ancora_filter_detect_inheritance_key','insurance_ancora_services_detect_inheritance_key', 9, 1);

		// Extra column for services lists
		if (insurance_ancora_get_theme_option('show_overriden_posts')=='yes') {
			add_filter('manage_edit-services_columns',			'insurance_ancora_post_add_options_column', 9);
			add_filter('manage_services_posts_custom_column',	'insurance_ancora_post_fill_options_column', 9, 2);
		}

		// Register shortcodes [trx_services] and [trx_services_item]
		add_action('insurance_ancora_action_shortcodes_list',		'insurance_ancora_services_reg_shortcodes');
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_services_reg_shortcodes_vc');
		
		// Add supported data types
		insurance_ancora_theme_support_pt('services');
		insurance_ancora_theme_support_tx('services_group');
	}
}

if ( !function_exists( 'insurance_ancora_services_settings_theme_setup2' ) ) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_services_settings_theme_setup2', 3 );
	function insurance_ancora_services_settings_theme_setup2() {
		// Add post type 'services' and taxonomy 'services_group' into theme inheritance list
		insurance_ancora_add_theme_inheritance( array('services' => array(
...
if ( !function_exists( 'insurance_ancora_is_services_page' ) ) {
	function insurance_ancora_is_services_page() {
		$is = in_array(insurance_ancora_storage_get('page_template'), array('blog-services', 'single-service'));
		if (!$is) {
			if (!insurance_ancora_storage_empty('pre_query'))
				$is = insurance_ancora_storage_call_obj_method('pre_query', 'get', 'post_type')=='services' 
						|| insurance_ancora_storage_call_obj_method('pre_query', 'is_tax', 'services_group') 
						|| (insurance_ancora_storage_call_obj_method('pre_query', 'is_page') 
								&& ($id=insurance_ancora_get_template_page_id('blog-services')) > 0 
				...
				$is = get_query_var('post_type')=='services' 
						...
						|| (is_page() && ($id=insurance_ancora_get_template_page_id('blog-services')) > 0 && $id==get_the_ID());
		}
		return $is;
	}
}
...
if ( !function_exists( 'insurance_ancora_services_detect_inheritance_key' ) ) {
	function insurance_ancora_services_detect_inheritance_key($key) {
		if (!empty($key)) return $key;
		return insurance_ancora_is_services_page() ? 'services' : '';
	}
}

// Filter to detect current page slug
if ( !function_exists( 'insurance_ancora_services_get_blog_type' ) ) {
	function insurance_ancora_services_get_blog_type($page, $query=null) {
...
if ( !function_exists( 'insurance_ancora_services_get_blog_title' ) ) {
	function insurance_ancora_services_get_blog_title($title, $page) {
		if (!empty($title)) return $title;
		if ( insurance_ancora_strpos($page, 'services')!==false ) {
...
if ( !function_exists( 'insurance_ancora_services_get_stream_page_title' ) ) {
	function insurance_ancora_services_get_stream_page_title($title, $page) {
		if (!empty($title)) return $title;
		if (insurance_ancora_strpos($page, 'services')!==false) {
			if (($page_id = insurance_ancora_services_get_stream_page_id(0, $page=='services' ? 'blog-services' : $page)) > 0)
...
if ( !function_exists( 'insurance_ancora_services_get_stream_page_id' ) ) {
	function insurance_ancora_services_get_stream_page_id($id, $page) {
		if (!empty($id)) return $id;
		if (insurance_ancora_strpos($page, 'services')!==false) $id = insurance_ancora_get_template_page_id('blog-services');
		return $id;
	}
}

// Filter to detect stream page URL
if ( !function_exists( 'insurance_ancora_services_get_stream_page_link' ) ) {
	function insurance_ancora_services_get_stream_page_link($url, $page) {
		if (!empty($url)) return $url;
		if (insurance_ancora_strpos($page, 'services')!==false) {
			$id = insurance_ancora_get_template_page_id('blog-services');
...
if ( !function_exists( 'insurance_ancora_services_get_current_taxonomy' ) ) {
	function insurance_ancora_services_get_current_taxonomy($tax, $page) {
...
if ( !function_exists( 'insurance_ancora_services_is_taxonomy' ) ) {
	function insurance_ancora_services_is_taxonomy($tax, $query=null) {
		if (!empty($tax))
			return $tax;
		else 
...
if ( !function_exists( 'insurance_ancora_services_query_add_filters' ) ) {
	function insurance_ancora_services_query_add_filters($args, $filter) {
		if ($filter == 'services') {
...
[/trx_services]
*/
if ( !function_exists( 'insurance_ancora_sc_services' ) ) {
	function insurance_ancora_sc_services($atts, $content=null){	
		...
		if (insurance_ancora_param_is_off($slider) && $columns > 1 && $style == 'services-5' && !empty($image)) $columns = 2;
		if (!empty($image)) {
			if ($image > 0) {
		...
		insurance_ancora_storage_set('sc_services_data', array(
			'id' => $id,
            'style' => $style,
            'type' => $type,
            'columns' => $columns,
        ...
        $output = '<div' . ($id ? ' id="'.esc_attr($id).'_wrap"' : '')
					...
					. '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
						...
						. (!insurance_ancora_param_is_off($equalheight) ? ' data-equal-height=".sc_services_item"' : '')
						. (!insurance_ancora_param_is_off($animation) ? ' data-animation="'.esc_attr(insurance_ancora_get_animation_classes($animation)).'"' : '')
					. '>'
					. (!empty($subtitle) ? '<h6 class="sc_services_subtitle sc_item_subtitle">' . trim(insurance_ancora_strmacros($subtitle)) . '</h6>' : '')
					. (!empty($title) ? '<h2 class="sc_services_title sc_item_title">' . trim(insurance_ancora_strmacros($title)) . '</h2>' : '')
					. (!empty($description) ? '<div class="sc_services_descr sc_item_descr">' . trim(insurance_ancora_strmacros($description)) . '</div>' : '')
		...
		if (insurance_ancora_param_is_on($custom) && $content) {
			...
			$args = insurance_ancora_query_add_posts_and_cats($args, $ids, 'services', $cat, 'services_group');
		...
		if (insurance_ancora_param_is_on($slider)) {
			$output .= '</div>'
				...
				. '<a class="sc_slider_next" href="#"></a></div>'
		...
		$output .=  (!empty($link) ? '<div class="sc_services_button sc_item_button">'.insurance_ancora_do_shortcode('[trx_button link="'.esc_url($link).'" icon="icon-right"]'.esc_html($link_caption).'[/trx_button]').'</div>' : '')
					. '</div><!-- /.sc_services -->'
		...
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_services', $atts, $content);
	}
	add_shortcode('trx_services', 'insurance_ancora_sc_services');
}


if ( !function_exists( 'insurance_ancora_sc_services_item' ) ) {
	function insurance_ancora_sc_services_item($atts, $content=null) {
		if (insurance_ancora_in_shortcode_blogger()) return '';
		extract(insurance_ancora_html_decode(shortcode_atts( array(
			// Individual params
			"icon" => "",
		...
		insurance_ancora_storage_inc_array('sc_services_data', 'counter');

		$id = $id ? $id : (insurance_ancora_storage_get_array('sc_services_data', 'id') ? insurance_ancora_storage_get_array('sc_services_data', 'id') . '_' . insurance_ancora_storage_get_array('sc_services_data', 'counter') : '');

		$descr = trim(chop(do_shortcode($content)));
		$readmore = $readmore=='(none)' ? insurance_ancora_storage_get_array('sc_services_data', 'readmore') : $readmore;

		$type = insurance_ancora_storage_get_array('sc_services_data', 'type');
		if (!empty($icon)) {
			$type = 'icons';
		} else if (!empty($image)) {
			$type = 'images';
			...
			$thumb_sizes = insurance_ancora_get_thumb_sizes(array('layout' => insurance_ancora_storage_get_array('sc_services_data', 'style')));
			$image = insurance_ancora_get_resized_image_tag($image, $thumb_sizes['w'], $thumb_sizes['h']);
		}
		...
		$args = array(
			'layout' => insurance_ancora_storage_get_array('sc_services_data', 'style'),
			'number' => insurance_ancora_storage_get_array('sc_services_data', 'counter'),
			'columns_count' => insurance_ancora_storage_get_array('sc_services_data', 'columns'),
			'slider' => insurance_ancora_storage_get_array('sc_services_data', 'slider'),
			'show' => false,
			...
			'tag_css' => $css,
			'tag_css_wh' => insurance_ancora_storage_get_array('sc_services_data', 'css_wh')
		);
		$output = insurance_ancora_show_post_layout($args, $post_data);
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_services_item', $atts, $content);
	}
	add_shortcode('trx_services_item', 'insurance_ancora_sc_services_item');
}
// ---------------------------------- [/trx_services] ---------------------------------------
...
if (!function_exists('insurance_ancora_services_reg_shortcodes')) {
	function insurance_ancora_services_reg_shortcodes() {
		if (insurance_ancora_storage_isset('shortcodes')) {

			$services_groups = insurance_ancora_get_list_terms(false, 'services_group');
			$services_styles = insurance_ancora_get_list_templates('services');
			$controls 		 = insurance_ancora_get_list_slider_controls();

			insurance_ancora_sc_map_after('trx_section', array(
				...
				"trx_services" => array(
					...
					"params" => array(
						...
						"cat" => array(
							...
							"style" => "list",
							"multiple" => true,
							"options" => insurance_ancora_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), $services_groups)
						),
						"count" => array(
...
if (!function_exists('insurance_ancora_services_reg_shortcodes_vc')) {
	function insurance_ancora_services_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"params" => array(
					...
					array(
						...
						"class" => "",
						"value" => array_flip((array)insurance_ancora_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), $services_groups)),
						"type" => "dropdown"
					),
					array(
						"param_name" => "columns",
		...
		class WPBakeryShortCode_Trx_Services extends INSURANCE_ANCORA_VC_ShortCodeColumns {}
		class WPBakeryShortCode_Trx_Services_Item extends INSURANCE_ANCORA_VC_ShortCodeCollection {}
