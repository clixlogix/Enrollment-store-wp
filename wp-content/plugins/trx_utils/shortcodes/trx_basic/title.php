...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('insurance_ancora_sc_title_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_sc_title_theme_setup' );
	function insurance_ancora_sc_title_theme_setup() {
		add_action('insurance_ancora_action_shortcodes_list', 		'insurance_ancora_sc_title_reg_shortcodes');
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_sc_title_reg_shortcodes_vc');
	}
}
...
if (!function_exists('insurance_ancora_sc_title')) {	
	function insurance_ancora_sc_title($atts, $content=null){	
		...
		$css .= insurance_ancora_get_css_dimensions_from_values($width)
			.($align && $align!='none' && !insurance_ancora_param_is_inherit($align) ? 'text-align:' . esc_attr($align) .';' : '')
		...
		$pic = $style!='iconed' 
			...
			: '<span class="sc_title_icon sc_title_icon_'.esc_attr($position).'  sc_title_icon_'.esc_attr($image_size).($icon!='' && $icon!='none' ? ' '.esc_attr($icon) : '').'"'.'>'
				...
				.(empty($picture) && $image && $image!='none' ? '<img src="'.esc_url(insurance_ancora_strpos($image, 'http')===0 ? $image : insurance_ancora_get_file_url('images/icons/'.($image).'.png')).'" alt="" />' : '')
				.'</span>';
		$output = '<h' . esc_attr($type) . ($id ? ' id="'.esc_attr($id).'"' : '')
				. ' class="sc_title sc_title_'.esc_attr($style)
					.($align && $align!='none' && !insurance_ancora_param_is_inherit($align) ? ' sc_align_' . esc_attr($align) : '')
				...
				. '</h' . esc_attr($type) . '>';
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_title', $atts, $content);
	}
	add_shortcode('trx_title', 'insurance_ancora_sc_title');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_title_reg_shortcodes' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list', 'insurance_ancora_sc_title_reg_shortcodes');
	function insurance_ancora_sc_title_reg_shortcodes() {
	
		insurance_ancora_sc_map("trx_title", array(
			...
			"params" => array(
				...
				"image" => array(
					...
					"type" => "images",
					"size" => "small",
					"options" => insurance_ancora_get_sc_param('images')
				),
				"picture" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_title_reg_shortcodes_vc' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list_vc', 'insurance_ancora_sc_title_reg_shortcodes_vc');
	function insurance_ancora_sc_title_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				...
				array(
					...
					"value" => insurance_ancora_get_sc_param('images'),
					"type" => "dropdown"
				),
				array(
					"param_name" => "picture",
		...
		class WPBakeryShortCode_Trx_Title extends Insurance_Ancora_VC_ShortCodeSingle {}
	}
}
?>