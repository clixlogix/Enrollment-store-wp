...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('insurance_ancora_sc_price_block_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_sc_price_block_theme_setup' );
	function insurance_ancora_sc_price_block_theme_setup() {
		add_action('insurance_ancora_action_shortcodes_list', 		'insurance_ancora_sc_price_block_reg_shortcodes');
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_sc_price_block_reg_shortcodes_vc');
	}
}
...
if (!function_exists('insurance_ancora_sc_price_block')) {	
	function insurance_ancora_sc_price_block($atts, $content=null){	
		...
		$content = do_shortcode(insurance_ancora_sc_clear_around($content));
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
			...
			. '</div>';
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_price_block', $atts, $content);
	}
	add_shortcode('trx_price_block', 'insurance_ancora_sc_price_block');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_price_block_reg_shortcodes' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list', 'insurance_ancora_sc_price_block_reg_shortcodes');
	function insurance_ancora_sc_price_block_reg_shortcodes() {
	
		insurance_ancora_sc_map("trx_price_block", array(
			...
			"params" => array(
				"style" => array(
					...
					"value" => 1,
					"options" => insurance_ancora_get_list_styles(1, 3),
					"type" => "checklist"
				),
				"title" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_price_block_reg_shortcodes_vc' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list_vc', 'insurance_ancora_sc_price_block_reg_shortcodes_vc');
	function insurance_ancora_sc_price_block_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"std" => 1,
					"value" => array_flip((array)insurance_ancora_get_list_styles(1, 3)),
					"type" => "dropdown"
				),
				array(
					"param_name" => "title",
				...
				array(
					...
					"class" => "",
					"value" => array_flip(insurance_ancora_get_sc_param('schemes')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "align",
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip(insurance_ancora_get_sc_param('float')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "content",
		...
		class WPBakeryShortCode_Trx_PriceBlock extends Insurance_Ancora_VC_ShortCodeSingle {}
	}
}
?>