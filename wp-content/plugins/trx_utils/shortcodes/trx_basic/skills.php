...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('insurance_ancora_sc_skills_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_sc_skills_theme_setup' );
	function insurance_ancora_sc_skills_theme_setup() {
		add_action('insurance_ancora_action_shortcodes_list', 		'insurance_ancora_sc_skills_reg_shortcodes');
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_sc_skills_reg_shortcodes_vc');
	}
}
...
if (!function_exists('insurance_ancora_sc_skills')) {	
	function insurance_ancora_sc_skills($atts, $content=null){	
		...
		insurance_ancora_storage_set('sc_skills_data', array(
			'counter' => 0,
            'columns' => 0,
            'height'  => 0,
            'type'    => $type,
            'pie_compact' => insurance_ancora_param_is_on($pie_compact) ? 'on' : 'off',
		...
		insurance_ancora_enqueue_diagram($type);
		if ($type!='arc') {
			if ($layout=='' || ($layout=='columns' && $columns<1)) $layout = 'rows';
			if ($layout=='columns') insurance_ancora_storage_set_array('sc_skills_data', 'columns', $columns);
			if ($type=='bar') {
		...
		if ($style) {
			...
			insurance_ancora_storage_set_array('sc_skills_data', 'style', $style);
		}
		insurance_ancora_storage_set_array('sc_skills_data', 'max', $max_value);
		insurance_ancora_storage_set_array('sc_skills_data', 'dir', $dir);
		insurance_ancora_storage_set_array('sc_skills_data', 'height', insurance_ancora_prepare_css_value($height));
		...
		if (!insurance_ancora_storage_empty('sc_skills_data', 'height') && (insurance_ancora_storage_get_array('sc_skills_data', 'type') == 'arc' || (insurance_ancora_storage_get_array('sc_skills_data', 'type') == 'pie' && insurance_ancora_param_is_on(insurance_ancora_storage_get_array('sc_skills_data', 'pie_compact')))))
			$css .= 'height: '.insurance_ancora_storage_get_array('sc_skills_data', 'height');
		$content = do_shortcode($content);
		$output = '<div id="'.esc_attr($id).'"' 
					. ' class="sc_skills sc_skills_' . esc_attr($type) 
						. ($type=='bar' ? ' sc_skills_'.esc_attr($dir) : '') 
						. ($type=='pie' ? ' sc_skills_compact_'.esc_attr(insurance_ancora_storage_get_array('sc_skills_data', 'pie_compact')) : '') 
				...
				. '>'
					...
					. ($type=='arc' 
						? ('<div class="sc_skills_legend">'.(insurance_ancora_storage_get_array('sc_skills_data', 'legend')).'</div>'
							. '<div id="'.esc_attr($id).'_diagram" class="sc_skills_arc_canvas"></div>'
							. '<div class="sc_skills_data" style="display:none;">' . (insurance_ancora_storage_get_array('sc_skills_data', 'data')) . '</div>'
						  )
						: '')
					. ($type=='pie' && insurance_ancora_param_is_on(insurance_ancora_storage_get_array('sc_skills_data', 'pie_compact'))
						? ('<div class="sc_skills_legend">'.(insurance_ancora_storage_get_array('sc_skills_data', 'legend')).'</div>'
				...
				. '</div>';
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_skills', $atts, $content);
	}
	add_shortcode('trx_skills', 'insurance_ancora_sc_skills');
}


if (!function_exists('insurance_ancora_sc_skills_item')) {	
	function insurance_ancora_sc_skills_item($atts, $content=null) {
		if (insurance_ancora_in_shortcode_blogger()) return '';
		extract(insurance_ancora_html_decode(shortcode_atts( array(
			// Individual params
			"title" => "",
		...
		insurance_ancora_storage_inc_array('sc_skills_data', 'counter');
		$ed = insurance_ancora_substr($value, -1)=='%' ? '%' : '';
		$value = str_replace('%', '', $value);
		if (insurance_ancora_storage_get_array('sc_skills_data', 'max') < $value) insurance_ancora_storage_set_array('sc_skills_data', 'max', $value);
		$percent = round($value / insurance_ancora_storage_get_array('sc_skills_data', 'max') * 100);
		$start = 0;
		$stop = $value;
		$steps = 100;
		$step = max(1, round(insurance_ancora_storage_get_array('sc_skills_data', 'max')/$steps));
		$speed = mt_rand(10,40);
		$animation = round(($stop - $start) / $step * $speed);
		$title_block = '<div class="sc_skills_info"><div class="sc_skills_label">' . ($title) . '</div></div>';
		$old_color = $color;
		if (empty($color)) $color = insurance_ancora_storage_get_array('sc_skills_data', 'color');
		if (empty($color)) $color = insurance_ancora_get_scheme_color('text_link', $color);
		if (empty($bg_color)) $bg_color = insurance_ancora_storage_get_array('sc_skills_data', 'bg_color');
		if (empty($bg_color)) $bg_color = insurance_ancora_get_scheme_color('bg_color', $bg_color);
		if (empty($border_color)) $border_color = insurance_ancora_storage_get_array('sc_skills_data', 'border_color');
		if (empty($border_color)) $border_color = insurance_ancora_get_scheme_color('bd_color', $border_color);;
		if (empty($style)) $style = insurance_ancora_storage_get_array('sc_skills_data', 'style');
		$style = max(1, min(4, $style));
		$output = '';
		if (insurance_ancora_storage_get_array('sc_skills_data', 'type') == 'arc' || (insurance_ancora_storage_get_array('sc_skills_data', 'type') == 'pie' && insurance_ancora_param_is_on(insurance_ancora_storage_get_array('sc_skills_data', 'pie_compact')))) {
			if (insurance_ancora_storage_get_array('sc_skills_data', 'type') == 'arc' && empty($old_color)) {
				$rgb = insurance_ancora_hex2rgb($color);
				$color = 'rgba('.(int)$rgb['r'].','.(int)$rgb['g'].','.(int)$rgb['b'].','.(1 - 0.1*(insurance_ancora_storage_get_array('sc_skills_data', 'counter')-1)).')';
			}
			insurance_ancora_storage_concat_array('sc_skills_data', 'legend', 
			...
			insurance_ancora_storage_concat_array('sc_skills_data', 'data', 
				'<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
					. ' class="'.esc_attr(insurance_ancora_storage_get_array('sc_skills_data', 'type')).'"'
					. (insurance_ancora_storage_get_array('sc_skills_data', 'type')=='pie'
						? ( ' data-start="'.esc_attr($start).'"'
							. ' data-stop="'.esc_attr($stop).'"'
							. ' data-step="'.esc_attr($step).'"'
							. ' data-steps="'.esc_attr($steps).'"'
							. ' data-max="'.esc_attr(insurance_ancora_storage_get_array('sc_skills_data', 'max')).'"'
							...
							. ' data-cutout="'.esc_attr(insurance_ancora_storage_get_array('sc_skills_data', 'pie_cutout')).'"'
			...
			$output .= (insurance_ancora_storage_get_array('sc_skills_data', 'columns') > 0 
							? '<div class="sc_skills_column column-1_'.esc_attr(insurance_ancora_storage_get_array('sc_skills_data', 'columns')).'">' 
							: '')
					. (insurance_ancora_storage_get_array('sc_skills_data', 'type')=='bar' && insurance_ancora_storage_get_array('sc_skills_data', 'dir')=='horizontal' ? $title_block : '')
					. '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
						. ' class="sc_skills_item' . ($style ? ' sc_skills_style_'.esc_attr($style) : '') 
							. (!empty($class) ? ' '.esc_attr($class) : '')
							. (insurance_ancora_storage_get_array('sc_skills_data', 'counter') % 2 == 1 ? ' odd' : ' even') 
							. (insurance_ancora_storage_get_array('sc_skills_data', 'counter') == 1 ? ' first' : '') 
							. '"'
						. (insurance_ancora_storage_get_array('sc_skills_data', 'height') !='' || $css 
							? ' style="' 
								. (insurance_ancora_storage_get_array('sc_skills_data', 'height') !='' 
										? 'height: '.esc_attr(insurance_ancora_storage_get_array('sc_skills_data', 'height')).';' 
										: '') 
								. ($css) 
								. '"' 
							: '')
			...
			if (in_array(insurance_ancora_storage_get_array('sc_skills_data', 'type'), array('bar', 'counter'))) {
				$output .= '<div class="sc_skills_count"' . (insurance_ancora_storage_get_array('sc_skills_data', 'type')=='bar' && $color ? ' style="background-color:' . esc_attr($color) . '; border-color:' . esc_attr($color) . '"' : '') . '>'
							...
							.'</div>'
						. '</div>';
			} else if (insurance_ancora_storage_get_array('sc_skills_data', 'type')=='pie') {
			...
			$output .= 
					  (insurance_ancora_storage_get_array('sc_skills_data', 'type')=='counter' ? $title_block : '')
					. '</div>'
					. (insurance_ancora_storage_get_array('sc_skills_data', 'type')=='bar' && insurance_ancora_storage_get_array('sc_skills_data', 'dir')=='vertical' || insurance_ancora_storage_get_array('sc_skills_data', 'type') == 'pie' ? $title_block : '')
					. (insurance_ancora_storage_get_array('sc_skills_data', 'columns') > 0 ? '</div>' : '');
		}
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_skills_item', $atts, $content);
	}
	add_shortcode('trx_skills_item', 'insurance_ancora_sc_skills_item');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_skills_reg_shortcodes' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list', 'insurance_ancora_sc_skills_reg_shortcodes');
	function insurance_ancora_sc_skills_reg_shortcodes() {
	
		insurance_ancora_sc_map("trx_skills", array(
			...
			"params" => array(
				...
				"dir" => array(
					...
					"type" => "checklist",
					"dir" => "horizontal",
					"options" => insurance_ancora_get_sc_param('dir')
				), 
				"style" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_skills_reg_shortcodes_vc' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list_vc', 'insurance_ancora_sc_skills_reg_shortcodes_vc');
	function insurance_ancora_sc_skills_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				...
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip((array)insurance_ancora_get_sc_param('dir')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "style",
		...
		class WPBakeryShortCode_Trx_Skills extends INSURANCE_ANCORA_VC_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_Skills_Item extends Insurance_Ancora_VC_ShortCodeSingle {}
	}
}
?>