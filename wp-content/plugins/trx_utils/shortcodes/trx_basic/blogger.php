...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('insurance_ancora_sc_blogger_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_sc_blogger_theme_setup' );
	function insurance_ancora_sc_blogger_theme_setup() {
		add_action('insurance_ancora_action_shortcodes_list', 		'insurance_ancora_sc_blogger_reg_shortcodes');
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_sc_blogger_reg_shortcodes_vc');
	}
}
...
if (!function_exists('insurance_ancora_sc_blogger')) {	
	function insurance_ancora_sc_blogger($atts, $content=null){	
		...
		extract(insurance_ancora_html_decode(shortcode_atts(array(
			...
			"location" => "default",
			"dir" => "horizontal",
			"hover" => insurance_ancora_get_theme_option('hover_style'),
			"hover_dir" => insurance_ancora_get_theme_option('hover_dir'),
			"scroll" => "no",
			"controls" => "no",
			"rating" => "no",
			"info" => "yes",
		...
		insurance_ancora_storage_set('sc_blogger_counter', 0);
		...
		if ($descr == '') $descr = insurance_ancora_get_custom_option('post_excerpt_maxlength'.($columns > 1 ? '_masonry' : ''));
	
		if (!insurance_ancora_param_is_off($scroll)) {
			insurance_ancora_enqueue_slider();
			if (empty($id)) $id = 'sc_blogger_'.str_replace('.', '', mt_rand());
		}
		
		$class = apply_filters('insurance_ancora_filter_blog_class',
					'sc_blogger'
					. ' layout_'.esc_attr($style)
					. ' template_'.esc_attr(insurance_ancora_get_template_name($style))
					. (!empty($class) ? ' '.esc_attr($class) : '')
					. ' ' . esc_attr(insurance_ancora_get_template_property($style, 'container_classes'))
					. ' sc_blogger_' . ($dir=='vertical' ? 'vertical' : 'horizontal')
					. (insurance_ancora_param_is_on($scroll) && insurance_ancora_param_is_on($controls) ? ' sc_scroll_controls sc_scroll_controls_type_bottom sc_scroll_controls_'.esc_attr($dir) : '')
					. ($descr == 0 ? ' no_description' : ''),
					array('style'=>$style, 'dir'=>$dir, 'descr'=>$descr)
		);
	
		$container = apply_filters('insurance_ancora_filter_blog_container', insurance_ancora_get_template_property($style, 'container'), array('style'=>$style, 'dir'=>$dir));
		...
		$container2 = apply_filters('insurance_ancora_filter_blog_container2', insurance_ancora_get_template_property($style, 'container2'), array('style'=>$style, 'dir'=>$dir));
		...
		$output = '<div'
			...
			. '>'
			. ($container_start)
			. (!empty($subtitle) ? '<h6 class="sc_blogger_subtitle sc_item_subtitle">' . trim(insurance_ancora_strmacros($subtitle)) . '</h6>' : '')
			. (!empty($title) ? '<h2 class="sc_blogger_title sc_item_title">' . trim(insurance_ancora_strmacros($title)) . '</h2>' : '')
			. (!empty($description) ? '<div class="sc_blogger_descr sc_item_descr">' . trim(insurance_ancora_strmacros($description)) . '</div>' : '')
			. ($container2_start)
			. ($style=='list' ? '<ul class="sc_list sc_list_style_iconed">' : '')
			. ($dir=='horizontal' && $columns > 1 && insurance_ancora_get_template_property($style, 'need_columns') ? '<div class="columns_wrap">' : '')
			. (insurance_ancora_param_is_on($scroll) 
		...
		if (insurance_ancora_get_template_property($style, 'need_isotope')) {
			if (!insurance_ancora_param_is_off($filters))
				$output .= '<div class="isotope_filters"></div>';
			if ($columns<1) $columns = insurance_ancora_substr($style, -1);
		...
		if (!insurance_ancora_param_is_off($only)) $args = insurance_ancora_query_add_filters($args, $only);
		...
		while ( $query->have_posts() ) { $query->the_post();
			...
			insurance_ancora_storage_inc('sc_blogger_counter');
	
			$args = array(
				'layout' => $style,
				'show' => false,
				'number' => insurance_ancora_storage_get('sc_blogger_counter'),
				'add_view_more' => false,
				...
				"readmore" => $readmore,
				"loadmore" => $loadmore,
				"reviews" => insurance_ancora_param_is_on($rating),
				"dir" => $dir,
				"scroll" => insurance_ancora_param_is_on($scroll),
				"info" => insurance_ancora_param_is_on($info),
				...
				// Get post data
				'strip_teaser' => false,
				'content' => insurance_ancora_get_template_property($style, 'need_content'),
				'terms_list' => !insurance_ancora_param_is_off($filters) || insurance_ancora_get_template_property($style, 'need_terms'),
				'filters' => insurance_ancora_param_is_off($filters) ? '' : $filters,
				'hover' => $hover,
				'hover_dir' => $hover_dir
			);
		...
		if (!insurance_ancora_param_is_off($filters)) {
			...
			if ($filters == 'categories') {			// Use categories as filter items
				...
				$portfolio_parent = $cat ? max(0, insurance_ancora_get_parent_taxonomy_by_property($cat, 'show_filters', 'yes', true, $taxonomy)) : 0;
				$args2 = array(
					'type'			=> $post_type,
		...
		$output	.= (insurance_ancora_param_is_on($scroll) 
				? '</div></div><div id="'.esc_attr($id).'_scroll_bar" class="sc_scroll_bar sc_scroll_bar_'.esc_attr($dir).' '.esc_attr($id).'_scroll_bar"></div></div>'
					. (!insurance_ancora_param_is_off($controls) ? '<div class="sc_scroll_controls_wrap"><a class="sc_scroll_prev" href="#"></a><a class="sc_scroll_next" href="#"></a></div>' : '')
				: '')
			. ($dir=='horizontal' && $columns > 1 && insurance_ancora_get_template_property($style, 'need_columns') ? '</div>' :  '')
			. ($style == 'list' ? '</ul>' : '')
			. ($container2_end)
			. (!empty($link) 
		...
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_blogger', $atts, $content);
	}
	add_shortcode('trx_blogger', 'insurance_ancora_sc_blogger');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_blogger_reg_shortcodes' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list', 'insurance_ancora_sc_blogger_reg_shortcodes');
	function insurance_ancora_sc_blogger_reg_shortcodes() {
	
		insurance_ancora_sc_map("trx_blogger", array(
			...
			"params" => array(
				...
				"style" => array(
					...
					"value" => "accordion",
					"type" => "select",
					"options" => insurance_ancora_get_sc_param('blogger_styles')
				),
				"filters" => array(
					...
					"dir" => "horizontal",
					"type" => "checklist",
					"options" => insurance_ancora_get_sc_param('filters')
				),
				"hover" => array(
					...
					"value" => "",
					"type" => "select",
					"options" => insurance_ancora_get_sc_param('hovers')
				),
				"hover_dir" => array(
					...
					"value" => "left_to_right",
					"type" => "select",
					"options" => insurance_ancora_get_sc_param('hovers_dir')
				),
				"dir" => array(
				...
				"post_type" => array(
					...
					"value" => "post",
					"type" => "select",
					"options" => insurance_ancora_get_sc_param('posts_types')
				),
				"ids" => array(
				...
				"only" => array(
					...
					"value" => "no",
					"type" => "select",
					"options" => insurance_ancora_get_sc_param('formats')
				),
				"scroll" => array(
				...
				"location" => array(
					...
					"value" => "default",
					"type" => "select",
					"options" => insurance_ancora_get_sc_param('locations')
				),
				"rating" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_blogger_reg_shortcodes_vc' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list_vc', 'insurance_ancora_sc_blogger_reg_shortcodes_vc');
	function insurance_ancora_sc_blogger_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"std" => "accordion",
					"class" => "",
					"value" => array_flip((array)insurance_ancora_get_sc_param('blogger_styles')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "filters",
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip((array)insurance_ancora_get_sc_param('filters')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "hover",
					...
					"class" => "",
					"value" => array_flip((array)insurance_ancora_get_sc_param('hovers')),
					'dependency' => array(
						'element' => 'style',
				...
				array(
					...
					"class" => "",
					"value" => array_flip((array)insurance_ancora_get_sc_param('hovers_dir')),
					'dependency' => array(
						'element' => 'style',
						'value' => array('portfolio_2','portfolio_3','portfolio_4','grid_2','grid_3','grid_4','square_2','square_3','square_4','short_2','short_3','short_4','colored_2','colored_3','colored_4')
					),
				...
				array(
					...
					"value" => array_flip((array)insurance_ancora_get_sc_param('locations')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "dir",
				...
				array(
					...
					"class" => "",
					"value" => array_flip((array)insurance_ancora_get_sc_param('posts_types')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "ids",
				...
				array(
					...
					"class" => "",
					"group" => esc_html__('Query', 'trx_utils'),
					"value" => array_flip((array)insurance_ancora_get_sc_param('formats')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "scroll",
		...
		class WPBakeryShortCode_Trx_Blogger extends Insurance_Ancora_VC_ShortCodeSingle {}
	}
}
?>