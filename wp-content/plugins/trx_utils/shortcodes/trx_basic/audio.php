...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('insurance_ancora_sc_audio_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_sc_audio_theme_setup' );
	function insurance_ancora_sc_audio_theme_setup() {
		add_action('insurance_ancora_action_shortcodes_list', 		'insurance_ancora_sc_audio_reg_shortcodes');
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_sc_audio_reg_shortcodes_vc');
	}
}
...
if (!function_exists('insurance_ancora_sc_audio')) {	
	function insurance_ancora_sc_audio($atts, $content = null) {
		...
		$audio = '<audio'
			...
			. ' src="'.esc_url($src).'"'
			. (insurance_ancora_param_is_on($controls) ? ' controls="controls"' : '')
			. (insurance_ancora_param_is_on($autoplay) && is_single() ? ' autoplay="autoplay"' : '')
			. ' width="'.esc_attr($width).'" height="'.esc_attr($height).'"'
			. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
			. ($data)
			. '></audio>';
		if ( insurance_ancora_get_custom_option('substitute_audio')=='no') {
			if (insurance_ancora_param_is_on($frame)) {
				$audio = insurance_ancora_get_audio_frame($audio, $image, $s);
			}
		} else {
			if ((isset($_GET['vc_editable']) && $_GET['vc_editable']=='true') && (isset($_POST['action']) && $_POST['action']=='vc_load_shortcode')) {
				$audio = insurance_ancora_substitute_audio($audio, false);
			}
		}
		if (insurance_ancora_get_theme_option('use_mediaelement')=='yes')
			wp_enqueue_script('wp-mediaelement');
		return apply_filters('insurance_ancora_shortcode_output', $audio, 'trx_audio', $atts, $content);
	}
	add_shortcode("trx_audio", "insurance_ancora_sc_audio");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_audio_reg_shortcodes' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list', 'insurance_ancora_sc_audio_reg_shortcodes');
	function insurance_ancora_sc_audio_reg_shortcodes() {
	
		insurance_ancora_sc_map("trx_audio", array(
			...
			"params" => array(
				...
				"controls" => array(
					...
					"value" => "show",
					"type" => "switch",
					"options" => insurance_ancora_get_sc_param('show_hide')
				),
				"autoplay" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_audio_reg_shortcodes_vc' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list_vc', 'insurance_ancora_sc_audio_reg_shortcodes_vc');
	function insurance_ancora_sc_audio_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Audio extends Insurance_Ancora_VC_ShortCodeSingle {}
	}
}
?>