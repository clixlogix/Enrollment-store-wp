...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('insurance_ancora_sc_columns_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_sc_columns_theme_setup' );
	function insurance_ancora_sc_columns_theme_setup() {
		add_action('insurance_ancora_action_shortcodes_list', 		'insurance_ancora_sc_columns_reg_shortcodes');
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_sc_columns_reg_shortcodes_vc');
	}
}
...
if (!function_exists('insurance_ancora_sc_columns')) {	
	function insurance_ancora_sc_columns($atts, $content=null){	
		...
		insurance_ancora_storage_set('sc_columns_data', array(
			'counter' => 1,
			'equal_selector' => '',
            'after_span2' => false,
            'after_span3' => false,
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="columns_wrap sc_columns'
					. ' columns_' . (insurance_ancora_param_is_on($fluid) ? 'fluid' : 'nofluid') 
					. (!empty($margins) && insurance_ancora_param_is_off($margins) ? ' no_margins' : '') 
					. ' sc_columns_count_' . esc_attr($count)
					. (!empty($class) ? ' '.esc_attr($class) : '') 
				. '"'
				. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
				. (!insurance_ancora_param_is_off($equalheight) ? ' data-equal-height="'.esc_attr(insurance_ancora_storage_get_array('sc_columns_data', 'equal_selector')).'"' : '')
				. (!insurance_ancora_param_is_off($animation) ? ' data-animation="'.esc_attr(insurance_ancora_get_animation_classes($animation)).'"' : '')
				. '>'
					. trim($content)
				. '</div>';
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_columns', $atts, $content);
	}
	add_shortcode('trx_columns', 'insurance_ancora_sc_columns');
}


if (!function_exists('insurance_ancora_sc_column_item')) {	
	function insurance_ancora_sc_column_item($atts, $content=null) {
		if (insurance_ancora_in_shortcode_blogger()) return '';
		extract(insurance_ancora_html_decode(shortcode_atts( array(
			// Individual params
			"span" => "1",
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') . ' class="column-'.($span > 1 ? esc_attr($span) : 1).'_'.esc_attr(insurance_ancora_storage_get_array('sc_columns_data', 'count')).' sc_column_item sc_column_item_'.esc_attr(insurance_ancora_storage_get_array('sc_columns_data', 'counter')) 
					. (!empty($class) ? ' '.esc_attr($class) : '')
					. (insurance_ancora_storage_get_array('sc_columns_data', 'counter') % 2 == 1 ? ' odd' : ' even') 
					. (insurance_ancora_storage_get_array('sc_columns_data', 'counter') == 1 ? ' first' : '') 
					. ($span > 1 ? ' span_'.esc_attr($span) : '') 
					. (insurance_ancora_storage_get_array('sc_columns_data', 'after_span2') ? ' after_span_2' : '') 
					. (insurance_ancora_storage_get_array('sc_columns_data', 'after_span3') ? ' after_span_3' : '') 
					. (insurance_ancora_storage_get_array('sc_columns_data', 'after_span4') ? ' after_span_4' : '') 
					. '"'
					. ($css!='' ? ' style="'.esc_attr($css).'"' : '')
					. (!insurance_ancora_param_is_off($animation) ? ' data-animation="'.esc_attr(insurance_ancora_get_animation_classes($animation)).'"' : '')
					. '>'
					...
					. '</div>';
		insurance_ancora_storage_inc_array('sc_columns_data', 'counter', $span);
		insurance_ancora_storage_set_array('sc_columns_data', 'after_span2', $span==2);
		insurance_ancora_storage_set_array('sc_columns_data', 'after_span3', $span==3);
		insurance_ancora_storage_set_array('sc_columns_data', 'after_span4', $span==4);
		insurance_ancora_storage_set_array('sc_columns_data', 'equal_selector', $bg_color!=='' || $bg_image !== '' ? '.sc_column_item_inner' : '.sc_column_item');
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_column_item', $atts, $content);
	}
	add_shortcode('trx_column_item', 'insurance_ancora_sc_column_item');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_columns_reg_shortcodes' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list', 'insurance_ancora_sc_columns_reg_shortcodes');
	function insurance_ancora_sc_columns_reg_shortcodes() {
	
		insurance_ancora_sc_map("trx_columns", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_columns_reg_shortcodes_vc' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list_vc', 'insurance_ancora_sc_columns_reg_shortcodes_vc');
	function insurance_ancora_sc_columns_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Columns extends INSURANCE_ANCORA_VC_ShortCodeColumns {}
		class WPBakeryShortCode_Trx_Column_Item extends INSURANCE_ANCORA_VC_ShortCodeCollection {}
	}
}
?>