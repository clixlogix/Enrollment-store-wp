...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('insurance_ancora_sc_call_to_action_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_sc_call_to_action_theme_setup' );
	function insurance_ancora_sc_call_to_action_theme_setup() {
		add_action('insurance_ancora_action_shortcodes_list', 		'insurance_ancora_sc_call_to_action_reg_shortcodes');
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_sc_call_to_action_reg_shortcodes_vc');
	}
}
...
if (!function_exists('insurance_ancora_sc_call_to_action')) {	
	function insurance_ancora_sc_call_to_action($atts, $content=null){	
		...
		if (!empty($image)) {
			$thumb_sizes = insurance_ancora_get_thumb_sizes(array('layout' => 'excerpt'));
			$image = !empty($video) 
				? insurance_ancora_get_resized_image_url($image, 190, 100)
				: insurance_ancora_get_resized_image_tag($image, 190, 100);
		}
	
		if (!empty($video)) {
			$video = '<video' . ($id ? ' id="' . esc_attr($id.'_video') . '"' : '') 
				. ' class="sc_video"'
				. ' src="' . esc_url(insurance_ancora_get_video_player_url($video)) . '"'
				...
				. '>'
				. '</video>';
			if (insurance_ancora_get_custom_option('substitute_video')=='no') {
				$video = insurance_ancora_get_video_frame($video, $image, '', '');
			} else {
		...
		$featured = ($style==1 && (!empty($content) || !empty($image) || !empty($video))
					? '<div class="sc_call_to_action_featured column-1_6">'
						. (!empty($content) 
							? $content 
							: (!empty($video) 
								? $video 
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="sc_call_to_action'
					. (insurance_ancora_param_is_on($accent) ? ' sc_call_to_action_accented' : '')
			...
			. '>'
				...
				. '<div class="sc_call_to_action_info'.($need_columns ? ' column-5_6' : '').'">'
					. (!empty($subtitle) ? '<h6 class="sc_call_to_action_subtitle sc_item_subtitle">' . trim(insurance_ancora_strmacros($subtitle)) . '</h6>' : '')
					. (!empty($title) ? '<h2 class="sc_call_to_action_title sc_item_title">' . trim(insurance_ancora_strmacros($title)) . '</h2>' : '')
					. (!empty($description) ? '<div class="sc_call_to_action_descr sc_item_descr">' . trim(insurance_ancora_strmacros($description)) . '</div>' : '')
					. ($style==1 ? $buttons : '')
				. '</div>'
				. ($style==2 && $align!='right' ? $buttons : '')
				. ($align=='right' ? $featured : '')
		...
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_call_to_action', $atts, $content);
	}
	add_shortcode('trx_call_to_action', 'insurance_ancora_sc_call_to_action');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_call_to_action_reg_shortcodes' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list', 'insurance_ancora_sc_call_to_action_reg_shortcodes');
	function insurance_ancora_sc_call_to_action_reg_shortcodes() {
	
		insurance_ancora_sc_map("trx_call_to_action", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_call_to_action_reg_shortcodes_vc' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list_vc', 'insurance_ancora_sc_call_to_action_reg_shortcodes_vc');
	function insurance_ancora_sc_call_to_action_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Call_To_Action extends INSURANCE_ANCORA_VC_ShortCodeCollection {}
	}
}
?>