...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('insurance_ancora_sc_promo_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_sc_promo_theme_setup' );
	function insurance_ancora_sc_promo_theme_setup() {
		add_action('insurance_ancora_action_shortcodes_list', 		'insurance_ancora_sc_promo_reg_shortcodes');
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_sc_promo_reg_shortcodes_vc');
	}
}
...
if (!function_exists('insurance_ancora_sc_promo')) {	
	function insurance_ancora_sc_promo($atts, $content=null){	
		...
		$text_width = insurance_ancora_strpos($image_width, '%')!==false
					...
					. '<div class="sc_promo_inner">'
						...
						. '<div class="sc_promo_block sc_align_'.esc_attr($text_align).'" style="'.esc_attr($css_text).'">'
							. '<div class="sc_promo_block_inner">'
									. (!empty($subtitle) ? '<h6 class="sc_promo_subtitle sc_item_subtitle">' . trim(insurance_ancora_strmacros($subtitle)) . '</h6>' : '')
									. (!empty($title) ? '<h2 class="sc_promo_title sc_item_title">' . trim(insurance_ancora_strmacros($title)) . '</h2>' : '')
									. (!empty($description) ? '<div class="sc_promo_descr sc_item_descr">' . trim(insurance_ancora_strmacros($description)) . '</div>' : '')
									. (!empty($content) ? '<div class="sc_promo_content">'.do_shortcode($content).'</div>' : '')
									. (!empty($link) ? '<div class="sc_promo_button sc_item_button">'.insurance_ancora_do_shortcode('[trx_button link="'.esc_url($link).'" icon="icon-right"]'.esc_html($link_caption).'[/trx_button]').'</div>' : '')
							. '</div>'
						. '</div>'
					. '</div>'
				. '</div>';
		...
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_promo', $atts, $content);
	}
	add_shortcode('trx_promo', 'insurance_ancora_sc_promo');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_promo_reg_shortcodes' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list', 'insurance_ancora_sc_promo_reg_shortcodes');
	function insurance_ancora_sc_promo_reg_shortcodes() {
	
		insurance_ancora_sc_map("trx_promo", array(
			...
			"params" => array(
				...
				"image_position" => array(
					...
					"type" => "checklist",
					"dir" => "horizontal",
					"options" => insurance_ancora_get_sc_param('hpos')
				),
				"image_width" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_promo_reg_shortcodes_vc' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list_vc', 'insurance_ancora_sc_promo_reg_shortcodes_vc');
	function insurance_ancora_sc_promo_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				...
				array(
					...
					"class" => "",
					"std" => 'left',
					"value" => array_flip((array)insurance_ancora_get_sc_param('hpos')),
					"type" => "dropdown"
				),
				array(
		...
		class WPBakeryShortCode_Trx_Promo extends INSURANCE_ANCORA_VC_ShortCodeCollection {}
	}
}
?>