...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('insurance_ancora_sc_video_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_sc_video_theme_setup' );
	function insurance_ancora_sc_video_theme_setup() {
		add_action('insurance_ancora_action_shortcodes_list', 		'insurance_ancora_sc_video_reg_shortcodes');
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_sc_video_reg_shortcodes_vc');
	}
}
...
if (!function_exists('insurance_ancora_sc_video')) {	
	function insurance_ancora_sc_video($atts, $content = null) {
		...
		if (empty($height) && empty($width)) {
			$width='100%';
			if (insurance_ancora_param_is_off(insurance_ancora_get_custom_option('substitute_video'))) $height="400";
		}
		$ed = insurance_ancora_substr($width, -1);
		...
		$css_bg = insurance_ancora_get_css_paddings_from_values($bg_top, $bg_right, $bg_bottom, $bg_left);
	
		if ($src=='' && $url=='' && isset($atts[0])) {
			$src = $atts[0];
		}
		$url = $src!='' ? $src : $url;
		if ($image!='' && insurance_ancora_param_is_off($image))
			$image = '';
		else {
			if (insurance_ancora_param_is_on($autoplay) && is_singular() && !insurance_ancora_storage_get('blog_streampage'))
				$image = '';
			else {
				if ($image > 0) {
				...
				if ($bg_image) {
					...
					if (!is_single() || !empty($image)) $image = insurance_ancora_get_resized_image_url(empty($image) ? get_the_ID() : $image, $thumb_sizes['w'], $thumb_sizes['h'], null, false, false, false);
				} else
					if (!is_single() || !empty($image)) $image = insurance_ancora_get_resized_image_url(empty($image) ? get_the_ID() : $image, $ed!='%' ? $width : null, $height);
				if (empty($image) && (!is_singular() || insurance_ancora_storage_get('blog_streampage')))	// || insurance_ancora_param_is_off($autoplay)))
					$image = insurance_ancora_get_video_cover_image($url);
			}
		}
		if ($bg_image > 0) {
		...
		$url = insurance_ancora_get_video_player_url($src!='' ? $src : $url);
		...
		$video = '<video' . ($id ? ' id="' . esc_attr($id) . '"' : '') 
			...
			. (($image && insurance_ancora_param_is_on(insurance_ancora_get_custom_option('substitute_video'))) || (insurance_ancora_param_is_on($autoplay) && is_singular() && !insurance_ancora_storage_get('blog_streampage')) ? ' autoplay="autoplay"' : '') 
			. ' controls="controls" loop="loop"'
			. '>'
			. '</video>';
		if (insurance_ancora_param_is_off(insurance_ancora_get_custom_option('substitute_video'))) {
			if (insurance_ancora_param_is_on($frame)) $video = insurance_ancora_get_video_frame($video, $image, $css, $css_bg);
		} else {
			if ((isset($_GET['vc_editable']) && $_GET['vc_editable']=='true') && (isset($_POST['action']) && $_POST['action']=='vc_load_shortcode')) {
				$video = insurance_ancora_substitute_video($video, $width, $height, false);
			}
		}
		if (insurance_ancora_get_theme_option('use_mediaelement')=='yes')
			wp_enqueue_script('wp-mediaelement');
		return apply_filters('insurance_ancora_shortcode_output', $video, 'trx_video', $atts, $content);
	}
	add_shortcode("trx_video", "insurance_ancora_sc_video");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_video_reg_shortcodes' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list', 'insurance_ancora_sc_video_reg_shortcodes');
	function insurance_ancora_sc_video_reg_shortcodes() {
	
		insurance_ancora_sc_map("trx_video", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_video_reg_shortcodes_vc' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list_vc', 'insurance_ancora_sc_video_reg_shortcodes_vc');
	function insurance_ancora_sc_video_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Video extends Insurance_Ancora_VC_ShortCodeSingle {}
	}
}
?>