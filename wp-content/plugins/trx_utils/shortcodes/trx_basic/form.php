...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('insurance_ancora_sc_form_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_sc_form_theme_setup' );
	function insurance_ancora_sc_form_theme_setup() {
		add_action('insurance_ancora_action_shortcodes_list', 		'insurance_ancora_sc_form_reg_shortcodes');
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_sc_form_reg_shortcodes_vc');
	}
}
...
if (!function_exists('insurance_ancora_sc_form')) {	
	function insurance_ancora_sc_form($atts, $content = null) {
		...
		insurance_ancora_enqueue_messages();	// Load core messages
	
		insurance_ancora_storage_set('sc_form_data', array(
			'id' => $id,
            'counter' => 0
            )
        );
		...
		$output = '<div ' . ($id ? ' id="'.esc_attr($id).'_wrap"' : '')
			...
			.'<div ' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="sc_form'
					...
					. (!empty($align) && !insurance_ancora_param_is_off($align) ? ' align'.esc_attr($align) : '') 
				...
				. '>'
					. (!empty($subtitle) 
						? '<h6 class="sc_form_subtitle sc_item_subtitle">' . trim(insurance_ancora_strmacros($subtitle)) . '</h6>' 
						: '')
					. (!empty($title) 
						? '<h2 class="sc_form_title sc_item_title">' . trim(insurance_ancora_strmacros($title)) . '</h2>' 
						: '')
					. (!empty($description) 
						? '<div class="sc_form_descr sc_item_descr">' . trim(insurance_ancora_strmacros($description)) . ($style == 1 ? do_shortcode('[trx_socials size="tiny" shape="round"][/trx_socials]') : '') . '</div>' 
						: '');
		
		$output .= insurance_ancora_show_post_layout(array(
												'layout' => $style,
												'id' => $id,
												'action' => $action,
												'content' => $content,
		...
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_form', $atts, $content);
	}
	add_shortcode("trx_form", "insurance_ancora_sc_form");
}

if (!function_exists('insurance_ancora_sc_form_item')) {	
	function insurance_ancora_sc_form_item($atts, $content=null) {
		if (insurance_ancora_in_shortcode_blogger()) return '';
		extract(insurance_ancora_html_decode(shortcode_atts( array(
			// Individual params
			"type" => "text",
		...
		insurance_ancora_storage_inc_array('sc_form_data', 'counter');
	
		$class .= ($class ? ' ' : '') . insurance_ancora_get_css_position_as_classes($top, $right, $bottom, $left);
		if (empty($id)) $id = insurance_ancora_storage_get_array('sc_form_data', 'id').'_'.insurance_ancora_storage_get_array('sc_form_data', 'counter');
...
//            }
			...
			wp_enqueue_script( 'jquery-picker', insurance_ancora_get_file_url('/js/picker/picker.js'), array('jquery'), null, true );
			wp_enqueue_script( 'jquery-picker-date', insurance_ancora_get_file_url('/js/picker/picker.date.js'), array('jquery'), null, true );
			...
			wp_enqueue_script( 'jquery-picker', insurance_ancora_get_file_url('/js/picker/picker.js'), array('jquery'), null, true );
			wp_enqueue_script( 'jquery-picker-time', insurance_ancora_get_file_url('/js/picker/picker.time.js'), array('jquery'), null, true );
		...
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_form_item', $atts, $content);
	}
	add_shortcode('trx_form_item', 'insurance_ancora_sc_form_item');
}

// AJAX Callback: Send contact form data
if ( !function_exists( 'insurance_ancora_sc_form_send' ) ) {
	function insurance_ancora_sc_form_send() {
	
		if ( !wp_verify_nonce( insurance_ancora_get_value_gp('nonce'), admin_url('admin-ajax.php') ) )
			die();
	
		$response = array('error'=>'');
		if (!($contact_email = insurance_ancora_get_theme_option('contact_email')) && !($contact_email = insurance_ancora_get_theme_option('admin_email'))) 
			$response['error'] = esc_html__('Unknown admin email!', 'trx_utils');
		else {
			$type = insurance_ancora_substr($_REQUEST['type'], 0, 7);
			parse_str($_POST['data'], $post_data);

			if (in_array($type, array('form_1', 'form_2'))) {
				$user_name	= insurance_ancora_strshort($post_data['username'],	100);
				$user_email	= insurance_ancora_strshort($post_data['email'],	100);
				$user_subj	= insurance_ancora_strshort($post_data['subject'],	100);
				$user_msg	= insurance_ancora_strshort($post_data['message'],	insurance_ancora_get_theme_option('message_maxlength_contacts'));
            ...
            $mail = insurance_ancora_get_theme_option('mail_function') == 'mail' ? 'mail' : 'wp_mail';
            if (is_email($contact_email) && !@$mail($contact_email, $subj, apply_filters('insurance_ancora_filter_form_send_message', $msg))) {
...
if ( !function_exists( 'insurance_ancora_sc_form_show_fields' ) ) {
	function insurance_ancora_sc_form_show_fields($fields) {
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_form_reg_shortcodes' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list', 'insurance_ancora_sc_form_reg_shortcodes');
	function insurance_ancora_sc_form_reg_shortcodes() {
	
		$pages = insurance_ancora_get_list_pages(false);

		insurance_ancora_sc_map("trx_form", array(
			...
			"params" => array(
				...
				"style" => array(
					...
					"divider" => true,
					"value" => 'form_custom',
					"options" => insurance_ancora_get_sc_param('forms'),
					"type" => "checklist"
				), 
				"scheme" => array(
			...
			"children" => array(
				...
				"params" => array(
					"type" => array(
						...
						"type" => "checklist",
						"dir" => "horizontal",
						"options" => insurance_ancora_get_sc_param('field_types')
					), 
					"name" => array(
					...
					"label_position" => array(
						...
						"type" => "checklist",
						"dir" => "horizontal",
						"options" => insurance_ancora_get_sc_param('label_positions')
					), 
					"top" => insurance_ancora_get_sc_param('top'),
					"bottom" => insurance_ancora_get_sc_param('bottom'),
					"left" => insurance_ancora_get_sc_param('left'),
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_form_reg_shortcodes_vc' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list_vc', 'insurance_ancora_sc_form_reg_shortcodes_vc');
	function insurance_ancora_sc_form_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"class" => "",
					"std" => "form_custom",
					"value" => array_flip((array)insurance_ancora_get_sc_param('forms')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "scheme",
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip((array)insurance_ancora_get_sc_param('field_types')),
					"type" => "dropdown"
				),
				array(
					"param_name" => "name",
				...
				array(
					...
					"class" => "",
					"value" => array_flip((array)insurance_ancora_get_sc_param('label_positions')),
					"type" => "dropdown"
				),
				insurance_ancora_get_vc_param('id'),
				insurance_ancora_get_vc_param('class'),
		...
		class WPBakeryShortCode_Trx_Form extends INSURANCE_ANCORA_VC_ShortCodeCollection {}
		class WPBakeryShortCode_Trx_Form_Item extends INSURANCE_ANCORA_VC_ShortCodeItem {}
	}
}
?>