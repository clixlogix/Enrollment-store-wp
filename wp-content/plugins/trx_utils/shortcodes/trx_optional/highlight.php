...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('insurance_ancora_sc_highlight_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_sc_highlight_theme_setup' );
	function insurance_ancora_sc_highlight_theme_setup() {
		add_action('insurance_ancora_action_shortcodes_list', 		'insurance_ancora_sc_highlight_reg_shortcodes');
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_sc_highlight_reg_shortcodes_vc');
	}
}
...
if (!function_exists('insurance_ancora_sc_highlight')) {	
	function insurance_ancora_sc_highlight($atts, $content=null){	
		...
		$css .= ($color != '' ? 'color:' . esc_attr($color) . ';' : '')
			...
			.($font_size != '' ? 'font-size:' . esc_attr(insurance_ancora_prepare_css_value($font_size)) . '; line-height: 1em;' : '');
		$output = '<span' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				...
				. '</span>';
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_highlight', $atts, $content);
	}
	add_shortcode('trx_highlight', 'insurance_ancora_sc_highlight');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_highlight_reg_shortcodes' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list', 'insurance_ancora_sc_highlight_reg_shortcodes');
	function insurance_ancora_sc_highlight_reg_shortcodes() {
	
		insurance_ancora_sc_map("trx_highlight", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_highlight_reg_shortcodes_vc' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list_vc', 'insurance_ancora_sc_highlight_reg_shortcodes_vc');
	function insurance_ancora_sc_highlight_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Highlight extends Insurance_Ancora_VC_ShortCodeSingle {}
	}
}
?>