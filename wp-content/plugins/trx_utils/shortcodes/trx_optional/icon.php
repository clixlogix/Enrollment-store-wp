...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('insurance_ancora_sc_icon_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_sc_icon_theme_setup' );
	function insurance_ancora_sc_icon_theme_setup() {
		add_action('insurance_ancora_action_shortcodes_list', 		'insurance_ancora_sc_icon_reg_shortcodes');
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_sc_icon_reg_shortcodes_vc');
	}
}
...
if (!function_exists('insurance_ancora_sc_icon')) {	
	function insurance_ancora_sc_icon($atts, $content=null){	
		...
		$css2 = ($font_weight != '' && !insurance_ancora_is_inherit_option($font_weight) ? 'font-weight:'. esc_attr($font_weight).';' : '')
			. ($font_size != '' ? 'font-size:' . esc_attr(insurance_ancora_prepare_css_value($font_size)) . '; line-height: ' . (!$bg_shape || insurance_ancora_param_is_inherit($bg_shape) ? '1' : '1.2') . 'em;' : '')
		...
		$output = $icon!='' 
			? ($link ? '<a href="'.esc_url($link).'"' : '<span') . ($id ? ' id="'.esc_attr($id).'"' : '')
				. ' class="sc_icon '.esc_attr($icon)
					. ($bg_shape && !insurance_ancora_param_is_inherit($bg_shape) ? ' sc_icon_shape_'.esc_attr($bg_shape) : '')
				...
				.($link ? '</a>' : '</span>')
			: '';
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_icon', $atts, $content);
	}
	add_shortcode('trx_icon', 'insurance_ancora_sc_icon');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_icon_reg_shortcodes' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list', 'insurance_ancora_sc_icon_reg_shortcodes');
	function insurance_ancora_sc_icon_reg_shortcodes() {
	
		insurance_ancora_sc_map("trx_icon", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_icon_reg_shortcodes_vc' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list_vc', 'insurance_ancora_sc_icon_reg_shortcodes_vc');
	function insurance_ancora_sc_icon_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Icon extends Insurance_Ancora_VC_ShortCodeSingle {}
	}
}
?>