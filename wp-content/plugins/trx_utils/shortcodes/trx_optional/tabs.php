...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('insurance_ancora_sc_tabs_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_sc_tabs_theme_setup' );
	function insurance_ancora_sc_tabs_theme_setup() {
		add_action('insurance_ancora_action_shortcodes_list', 		'insurance_ancora_sc_tabs_reg_shortcodes');
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_sc_tabs_reg_shortcodes_vc');
	}
}
...
if (!function_exists('insurance_ancora_sc_tabs')) {	
	function insurance_ancora_sc_tabs($atts, $content = null) {
		...
		insurance_ancora_storage_set('sc_tab_data', array(
			'counter'=> 0,
            'scroll' => $scroll,
		...
		$sc_tab_titles = insurance_ancora_storage_get_array('sc_tab_data', 'titles');
		...
		$tabs_output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
							...
							. '>'
						.'<ul class="sc_tabs_titles clearfix">';
		$titles_output = '';
		...
		$tabs_output .= $titles_output
			...
			. ($content)
			.'</div>';
		return apply_filters('insurance_ancora_shortcode_output', $tabs_output, 'trx_tabs', $atts, $content);
	}
	add_shortcode("trx_tabs", "insurance_ancora_sc_tabs");
}


if (!function_exists('insurance_ancora_sc_tab')) {	
	function insurance_ancora_sc_tab($atts, $content = null) {
		if (insurance_ancora_in_shortcode_blogger()) return '';
		extract(insurance_ancora_html_decode(shortcode_atts(array(
			// Individual params
		...
		insurance_ancora_storage_inc_array('sc_tab_data', 'counter');
		$counter = insurance_ancora_storage_get_array('sc_tab_data', 'counter');
		if (empty($id))
			$id = !empty($tab_id) ? $tab_id : insurance_ancora_storage_get_array('sc_tab_data', 'id').'_'.intval($counter);
		$sc_tab_titles = insurance_ancora_storage_get_array('sc_tab_data', 'titles');
		...
		insurance_ancora_storage_set_array('sc_tab_data', 'titles', $sc_tab_titles);
		$output = '<div id="'.esc_attr($id).'"'
					.' class="sc_tabs_content' 
						...
						. ($css!='' ? ' style="'.esc_attr($css).'"' : '') 
						. '>' 
				. (insurance_ancora_param_is_on(insurance_ancora_storage_get_array('sc_tab_data', 'scroll')) 
					? '<div id="'.esc_attr($id).'_scroll" class="sc_scroll sc_scroll_vertical" style="height:'.(insurance_ancora_storage_get_array('sc_tab_data', 'height') != '' ? insurance_ancora_storage_get_array('sc_tab_data', 'height') : '200px').';"><div class="sc_scroll_wrapper swiper-wrapper"><div class="sc_scroll_slide swiper-slide">' 
					: '')
				...
				. (insurance_ancora_param_is_on(insurance_ancora_storage_get_array('sc_tab_data', 'scroll')) 
					...
					: '')
			. '</div>';
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_tab', $atts, $content);
	}
	add_shortcode("trx_tab", "insurance_ancora_sc_tab");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_tabs_reg_shortcodes' ) ) {
	function insurance_ancora_sc_tabs_reg_shortcodes() {
	
		insurance_ancora_sc_map("trx_tabs", array(
			...
			"params" => array(
				"style" => array(
					...
					"value" => 1,
					"options" => insurance_ancora_get_list_styles(1, 2),
					"type" => "radio"
				),
				"initial" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_tabs_reg_shortcodes_vc' ) ) {
	function insurance_ancora_sc_tabs_reg_shortcodes_vc() {
		...
		vc_map( array(
			...
			"params" => array(
				array(
					...
					"admin_label" => true,
					"class" => "",
					"value" => array_flip((array)insurance_ancora_get_list_styles(1, 2)),
					"type" => "dropdown"
				),
				array(
					"param_name" => "initial",
		...
		class WPBakeryShortCode_Trx_Tabs extends INSURANCE_ANCORA_VC_ShortCodeTabs {}
		class WPBakeryShortCode_Trx_Tab extends INSURANCE_ANCORA_VC_ShortCodeTab {}
	}
}
?>