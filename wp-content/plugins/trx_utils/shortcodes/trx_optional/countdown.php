...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('insurance_ancora_sc_countdown_theme_setup')) {
	add_action( 'insurance_ancora_action_before_init_theme', 'insurance_ancora_sc_countdown_theme_setup' );
	function insurance_ancora_sc_countdown_theme_setup() {
		add_action('insurance_ancora_action_shortcodes_list', 		'insurance_ancora_sc_countdown_reg_shortcodes');
		if (function_exists('insurance_ancora_exists_visual_composer') && insurance_ancora_exists_visual_composer())
			add_action('insurance_ancora_action_shortcodes_list_vc','insurance_ancora_sc_countdown_reg_shortcodes_vc');
	}
}
...
if (!function_exists('insurance_ancora_sc_countdown')) {	
	function insurance_ancora_sc_countdown($atts, $content = null) {
		...
		wp_enqueue_script( 'insurance-ancora-jquery-plugin-script', insurance_ancora_get_file_url('js/countdown/jquery.plugin.js'), array('jquery'), null, true );
		wp_enqueue_script( 'insurance-ancora-countdown-script', insurance_ancora_get_file_url('js/countdown/jquery.countdown.js'), array('jquery'), null, true );
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
			...
			. '>'
				...
				. ($align=='center' ? '</div>' : '')
			. '</div>';
		return apply_filters('insurance_ancora_shortcode_output', $output, 'trx_countdown', $atts, $content);
	}
	add_shortcode("trx_countdown", "insurance_ancora_sc_countdown");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_countdown_reg_shortcodes' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list', 'insurance_ancora_sc_countdown_reg_shortcodes');
	function insurance_ancora_sc_countdown_reg_shortcodes() {
	
		insurance_ancora_sc_map("trx_countdown", array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'insurance_ancora_sc_countdown_reg_shortcodes_vc' ) ) {
	//add_action('insurance_ancora_action_shortcodes_list_vc', 'insurance_ancora_sc_countdown_reg_shortcodes_vc');
	function insurance_ancora_sc_countdown_reg_shortcodes_vc() {
		...
		class WPBakeryShortCode_Trx_Countdown extends Insurance_Ancora_VC_ShortCodeSingle {}
	}
}
?>